Feature: BestBuy Application
  ...
  Scenario Outline: Create a new store & verify if the store is added
    ...
    When I create a new Store by providing the information name "<name>", address "<address>", city "<city>", state "<state>", zip "<zip>", type  "<type>", address- "<address2>", lat "<lat>",lng "<lng>"
    Then I verify that the store  with lat "<lat>" is created
    Examples:
      | name  | address       | city | state | zip   | type | address2 | lat | lng |
      | mann1 | prime Testing | abcd | auto  | 102df | best | avenue   | 10  | 100 |

  Scenario: Update an exciting store & verify that store is updated
    When I update a store with storeId and give information storeId "39" name "mann", address "testing", city "southAmerica", state "Newyork", zip "we3wr", type  "abcd", address- "highway", lat "20",lng "10"
