...
public class StoreStepdefs {
    ...
    @When("^I create a new Store by providing the information name \"([^\"]*)\", address \"([^\"]*)\", city \"([^\"]*)\", state \"([^\"]*)\", zip \"([^\"]*)\", type  \"([^\"]*)\", address- \"([^\"]*)\", lat \"([^\"]*)\",lng \"([^\"]*)\"$")
    public void iCreateANewStoreByProvidingTheInformationNameAddressCityStateZipTypeAddressLatLngHoursServices(String name, String address, String city, String state, String zip, String type, String address2, String lat, String lng) {
      ...
      //  Map<String, Object> servicesList = new HashMap<>();
       // servicesList.put("name", services1);
        response = storeSteps.createStore(name, address, city, state, zip, type, address2, Integer.parseInt(lat), Integer.parseInt(lng));
    }
    ...
    @When("^I update a store with storeId and give information storeId \"([^\"]*)\" name \"([^\"]*)\", address \"([^\"]*)\", city \"([^\"]*)\", state \"([^\"]*)\", zip \"([^\"]*)\", type  \"([^\"]*)\", address- \"([^\"]*)\", lat \"([^\"]*)\",lng \"([^\"]*)\"$")
